...
강제 수준
- 본 구조체 필수 필드는 구현에서 생략할 수 없다.
- 선택 필드는 필요한 스킬에서만 사용한다.
...
5. Teleport는 목표 지점 + 주변 대체 지점 탐색이 모두 실패하면 취소한다.
6. 취소 시 SkillExecutionResult.FailCode=SKX_TeleportNoValidDestination을 반환한다.
