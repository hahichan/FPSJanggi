...
- 물리 컴포넌트는 단일(yjh_ArenaCombatComponent)로 간다.

3. 데이터 정책
- SkillDataAsset: 고정값(규칙/기준값)
- Runtime Request Context: 실시간값(위치/방향/히트)
- 실시간값을 SkillDataAsset 원본에 쓰는 행위 금지
- Runtime Request Context 구조체는 강제 계약으로 사용한다.

추가 고정값
- DefaultMaxSkillSlots=10 (설정값으로 확장 가능)
- Cooldown 상한=999.0초
- Teleport 유효 지점 없음 시 취소 + SKX_TeleportNoValidDestination 반환
- 패시브 슬롯은 UI에 항상 표시(배지 표시, 쿨타임 숫자 숨김)

4. 권한 정책
- 최종 판정/쿨타임 확정/승패 확정은 서버 권한으로만 수행
- 클라이언트는 요청만 수행하고 최종 결과는 서버 응답을 따른다.
...
## 3. 필수 문서 읽기 순서
1. ../yjh_ai_logs/yjh_agent_execution_plan.md
...
## 5. 변경 관리
- 본 기준서 변경 시 ../yjh_ai_logs/yjh_chat_3.md에 작업 번호로 기록
