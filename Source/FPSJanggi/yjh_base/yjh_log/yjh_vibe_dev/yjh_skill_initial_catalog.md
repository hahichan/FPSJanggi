...
## 0. 목적
- 스킬 실행기 검증을 위한 예시 스킬 조합을 제공한다.
- Slot1..Slot10 기준 슬롯 운용 예시를 보여주며, 캐릭터별 실제 조합은 자유다.
...
주의
- 본 표는 "예시 조합"이며 강제 스킬 세트가 아니다.
- 실제 캐릭터는 Slot별 SkillData 조합을 자유롭게 구성한다.
...
Reserved 의미
- Reserved/Unassigned 슬롯은 입력이 들어와도 실행되지 않는 비활성 슬롯이다.
- 시스템은 해당 입력을 무시하거나 실패코드 이벤트만 발생시킨다.
