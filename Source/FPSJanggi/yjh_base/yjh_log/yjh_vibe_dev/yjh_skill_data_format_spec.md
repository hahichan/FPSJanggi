...
- 슬롯 수는 코드 상수 하드코딩이 아니라 설정값(MaxSkillSlots)으로 관리한다.
...
프로젝트 고정 설정
- DefaultMaxSkillSlots = 10
- MaxSkillSlots는 설정값으로 관리하며, 코드 수정 없이 증가 가능한 구조를 유지한다.
- CooldownSec 유효 범위는 0.0 ~ 999.0
...
- 쿨타임 조작/설정 후 값은 항상 [0.0, 999.0] 범위로 클램프한다.
...
- CooldownControl 실행 결과는 최대 999.0으로 클램프(상한 고정)
